clientsApp.controller('createClientController', ['$scope', 'clientPostDataService', '$stateParams', '$state', '$timeout' ,'$modal'
     , function createClientController($scope, clientPostDataService, $stateParams, $state, $timeout ,$modal) {
        ...
        if (clientDataToedit) { //if true then edit page
            ...
            $scope.finalContactArray.forEach(function (obj) {
                return obj.ClientId = clientDataToedit.ClientId
            }); //Add ClientId into finalContactArray
            $scope.clientDataObj = {
        ...
        $scope.removeUploadedImg = function () {
                var clientId = {
                    CompanyId: angular.element(document.querySelector('#uploadedImg')).attr('data-attr')
                }
